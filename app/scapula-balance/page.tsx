...
const scapulaFunctions = ["Protraction", "Retraction", "Elevation", "Depression"]
...
const generateInitialExercises = (): Exercise[] => {
  let id = 1
  return scapulaFunctions.flatMap((func) =>
    ranges.map((range) => ({
      id: id++,
      exercises: [
        {
          name: `${range === "short" ? "Short" : "Full"} Range ${func} Exercise 1`,
          description: `Strengthen your scapular ${func.toLowerCase()} with ${
        ...
        },
        {
          ...
          description: `Isolate your scapular ${func.toLowerCase()} with ${
            range === "short" ? "limited" : "full"
          } range of motion`,
          function: func,
          range: range,
...
export default function ScapulaBalance() {
  ...
  const { stopCountdown } = useCountdown("scapula")

  useEffect(() => {
    const loadExercises = () => {
      setIsLoading(true)
      try {
        const storedExercises = localStorage.getItem("scapulaExercises")
        if (storedExercises) {
          setExercises(JSON.parse(storedExercises))
        } else {
          const initialExercises = generateInitialExercises()
          setExercises(initialExercises)
          localStorage.setItem("scapulaExercises", JSON.stringify(initialExercises))
        }
      } catch (error) {
        setError("Failed to load scapula exercises. Please try again.")
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleSaveNewExercise = () => {
    ...
    localStorage.setItem("scapulaExercises", JSON.stringify(updatedExercises))
  ...
  const handleExerciseComplete = (id: number) => {
    ...
    localStorage.setItem("scapulaExercises", JSON.stringify(exercises))
    stopCountdown(id)
  }

  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">Scapula Balance</h1>
      <Card className="mb-6">
        <CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex justify-between items-center flex-wrap gap-4">
                <span>Scapula Exercises To-Do List</span>
