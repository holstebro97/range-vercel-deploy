...
const kneeFunctions = ["Knee Flexion", "Knee Extension", "Hip Flexion"]
...
const generateInitialExercises = (): Exercise[] => {
  let id = 1
  return kneeFunctions.flatMap((func) =>
    ranges.map((range) => ({
      id: id++,
      exercises: [
        {
          name: `${range === "short" ? "Short" : "Full"} Range ${func} Exercise 1`,
          description: `Strengthen your ${func.toLowerCase()} muscles with ${range === "short" ? "limited" : "full"} range of motion`,
          function: func,
          range: range,
        ...
        },
        {
          ...
          description: `Isolate your ${func.toLowerCase()} muscles with ${range === "short" ? "limited" : "full"} range of motion`,
          function: func,
          range: range,
          youtubeLink: "https://www.youtube.com/watch?v=example",
...
export default function KneeBalance() {
  ...
  const { stopCountdown } = useCountdown("knee")

  useEffect(() => {
    const loadExercises = () => {
      setIsLoading(true)
      try {
        const storedExercises = localStorage.getItem("kneeExercises")
        const storedVersion = localStorage.getItem("kneeExercisesVersion")
        ...
        if (storedExercises && storedVersion === "3") {
          ...
          localStorage.setItem("kneeExercises", JSON.stringify(initialExercises))
          localStorage.setItem("kneeExercisesVersion", "3")
        }
      } catch (error) {
        setError("Failed to load knee exercises. Please try again.")
      } finally {
        setIsLoading(false)
      }
    }
  ...
  const handleSaveNewExercise = () => {
    ...
    localStorage.setItem("kneeExercises", JSON.stringify(updatedExercises))
    localStorage.setItem("kneeExercisesVersion", "3")
  ...
  const handleExerciseComplete = (id: number) => {
    ...
    localStorage.setItem("kneeExercises", JSON.stringify(exercises))
    stopCountdown(id)
  }

  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-6 text-gray-800">Knee Balance</h1>
      <Card className="mb-6">
        <CardHeader>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex justify-between items-center flex-wrap gap-4">
                <span>Knee Exercises To-Do List</span>
